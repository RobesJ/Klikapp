...
export const generateCleaningRecord = async (
  ...
  user: User,
  client: Client,
  object: ObjectWithRelations['object'],
  chimney: Chimney,
    ...
    const html = `
      ...
      <html>
        ...
        <body>
          ...
          <!-- INFO LIST -->
          <div class="info-list">
            <p class="info-row"><span class="label">Adresa objektu:</span> ${object.address}</p>
            ${clientInfoRows}
...
export const generateAllChimneyRecords = async (
  project: Project,
  user: User,
  client: Client,
  ...
  watermarkBase64: string,
  footerImageBase64: string,
  recordType: 'cleaning' | 'inspection'
) => {
  const uris: string[] = [];
  
  for (const obj of objects) {
    for (let i = 0; i < obj.chimneys.length; i++) {
      const chimney = obj.chimneys[i];
      const chimneyNumber = i + 1;
      ...
      try {
        ...
        console.error(`Error generating record for chimney ${chimneyNumber}:`, error);
      }
    }
  }
